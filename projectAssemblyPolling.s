...
la           $t0,0xffff0008     #load address of the register Transmitter control(screen) to read the character
	
lw           $t0,0($t0)         #$t0=ready bit
andi         $t0,$t0,1          #compare ready bit with 1 via and operation
beq          $t0,$zero,printChar#check if the ready bit is 0
la           $t0,0xffff000c     #write the character from receiver data in Transmitter data
 
sb           $a0,0($t0)         #in order to print the character from the user we must load it on $a0
...
la           $t2,0xffff0000     #load address of the register Receiver control(keyboard) 

lw           $t2,0($t2)         #$t2=ready bit
andi         $t2,$t2,1	        #compare ready bit with 1 via and operation
beq          $t2,$zero,readChar #check if the ready bit is 0
la           $t2,0xffff0004     #load the character from Receiver data in Receiver data

lb           $v0,0($t2)         #in order to return the character from the user we must load it on $v0
...
beq          $a0,0,endLoop 		# check if this character is null 			
beq          $a0,10,endLoop		# check if this character is \0
